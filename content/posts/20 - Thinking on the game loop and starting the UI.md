...
<p style="text-align:center; font-style:italic;">My first iteration on the game loop</p>
...
In Unreal, I created a basic level called MainMenu. In the level in copied the sky and the lights parameters from the TinyTown map to have the same ambiance when I position a main menu scene. Then, I created a user widget blueprint called `WBP_MainMenu` and in the level blueprint I added it to the game on `BeginPlay`. I also set the input mode to UI only.
